...
import { useGetAllNotice } from "../../api-hook";
import { Edit, Eye, Trash } from "lucide-react";
import Delete from "@/components/ui/Delete";
import { deleteNotice } from "../../api-hook/mutations";
...
export default function Notice() {
  ...
  const { data, isLoading, refetch } = useGetAllNotice();
  ...
  const handleDelete = async () => {
    try {
      const checkIsLastItem =
        data.find((el) => el._id === selectedRow._id).notices.length > 1;
      await deleteNotice(
        selectedRow._id,
            ...
            {data
              ...
              ?.map((el) => (
                <React.Fragment key={el?._id}>
                  <TableRow>
                    <TableCell rowSpan={el.notices.length + 1}>
                      {el.financialYear}
                    </TableCell>
                    <TableCell rowSpan={el.notices.length + 1}>
                      {el.heading}
                      ...
                      <TableCell>
                        <div className="flex gap-4">
                          ...
                          <a
                            ...
                            href={UPLOADED_IMAGE_PATH + notice.file.filePath}
                            target="_blank"
