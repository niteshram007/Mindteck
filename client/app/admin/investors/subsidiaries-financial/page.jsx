...
import AddSubsidiariesFinancial from "./actions";
import { useGetAllSubsidiariesFinancial } from "../../api-hook";
import { Edit, Eye, Trash } from "lucide-react";
import Delete from "@/components/ui/Delete";
import { deleteSubsidiariesFinancial } from "../../api-hook/mutations";
...
export default function SubsidiariesFinancial() {
  ...
  const { data, isLoading, refetch } = useGetAllSubsidiariesFinancial();
  ...
  const handleDelete = async () => {
    try {
      const checkIsLastItem =
        data.find((el) => el._id === selectedRow._id).subsidiaries.length > 1;
      await deleteSubsidiariesFinancial(
        selectedRow._id,
            ...
            {data
              ...
              ?.map((el) => (
                <React.Fragment key={el?._id}>
                  <TableRow>
                    <TableCell rowSpan={el.subsidiaries.length + 1}>
                      {el.financialYear}
                    </TableCell>
                  </TableRow>
                  {el?.subsidiaries?.map((item) => (
                    <TableRow key={item.subsidiary}>
                      ...
                      <TableCell>
                        <div className="flex gap-4">
                          ...
                          <a rel="noopener noreferrer"
                            href={UPLOADED_IMAGE_PATH + item.file.filePath}
                            target="_blank"
