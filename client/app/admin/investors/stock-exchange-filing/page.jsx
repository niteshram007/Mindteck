...
import { useGetAllStockExchangeFilling } from "../../api-hook";
import { Edit, Eye, Trash } from "lucide-react";
import Delete from "@/components/ui/Delete";
import { deleteStockExchangeFiling } from "../../api-hook/mutations";
...
export default function StockExchangeFilling() {
  const { toast } = useToast();
  const { data, isLoading, refetch } = useGetAllStockExchangeFilling();
  ...
  const handleDelete = async () => {
    try {
      const checkIsLastItem =
        data.find((el) => el._id === selectedRow._id).stockExchangeFilings
          .length > 1;
      await deleteStockExchangeFiling(
        selectedRow._id,
            ...
            {data
              ...
              ?.map((el) => (
                <React.Fragment key={el?._id}>
                  <TableRow>
                    <TableCell rowSpan={el.stockExchangeFilings.length + 1}>
                      {el.financialYear}
                    </TableCell>
                  </TableRow>
